...
public class MenuTest extends Group {
    ...
    public MenuTest(ResumeButtonListener listener, GameStateManager gsm, Stage parentStage) {
...
//    }
    ...
    private void setSettingMenu() {
        ...
        settingMenu.addListener(new ClickListener() {
            @Override
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                sequenceSetting.addAction(Actions.delay(0.3f));
                sequenceSetting.addAction(new Action() {
                    @Override
                    public boolean act(float delta) {
                        getStage().addActor(new MenuSetting(gsm));
                        return true;
                    }
                });
